# Script Parameters
Here you can find a list of the parameters you can pass to ff2ppress, explanations on what each of them does, and usage examples.
...
Most of these parameters have default values, which will be mentioned here if applicable.
...
**Default:** Same folder as the input video
...
**Default:** Depends on the selected encoder. See [Encoders.md](Encoders.md)

The video preset for the selected encoder. Each encoder may have different preset naming schemes, so check the supported presets for each [encoder](Encoders.md).
...
**Example:**\
Each encoder has their own "presets" which is the main decider between the encoding speed and quality. Presets are not comparable between encoders, for example `libx265`'s `medium` preset will not achieve the same quality or speed as `libx264`'s `medium` preset.

**Usage:**\
`-cvpreset veryfast` (this is a valid preset for `libx265` and `libx264`)\
`-cvpreset p5` (this is a valid preset for `hevc_nvenc`, `h264_nvenc`, `av1_nvenc`)\
`-cvpreset 7` (valid for `libsvtav1` for example)
...
`-params enable-variance-boost=1:variance-boost-strength=1`\
This will correctly pass `-svtav1-params enable-variance-boost=1:variance-boost-strength=1` to FFmpeg.

If encoders add their own "Extra FFmpeg parameters" FF2ppress will merge any -params you pass. For example, when using [libsvtav1](Encoders.md#libsvtav1) the script automatically adds `-svtav1-params lookahead=42`, but you can still use `-params enable-variance-boost=1` and your options will get merged with the script's options, resulting into: `-svtav1-params lookahead=42:enable-variance-boost=1`

**Usage:**\
`-params lp=4:enable-variance-boost=1` (these are parameters for `libsvtav1` as an example)

### -TargetVideoHeight (Alias: -h) & -TargetVideoWidth (Alias: -w)
**Default:** `-1` (does not rescale the video)

Set a video height or width in pixels to rescale the video.

**Example:**\
You can use either `-h` or `-w`, and the unset value will scale correctly to keep the aspect ratio of the video.
...
`-h 720`\
`-w 1920`\
`-h 1080 -w 1920` (you can set both values, but its unnecessary)\
...
Set a start and end timestamp separated by "-" like so: `-trim <start_timestamp>-<end_timestamp>`, to keep that section of the video while the rest will be trimmed out.
...
When using [-trim](Parameters.md#-targetvideotrim-alias--trim), there could be a chance that the target video bitrate will end up being higher than the starting video target. This (roughly) means trimming the video will be enough to get under the target file size without having to re-encode the video.
...
**Example:**\
Sometimes the output video size overshoots the target size, and the script will have to [retry](Parameters.md#-retryencodingiftargetnotmet-alias--retry-bool) with a lower video bitrate. If you want to make sure the video will get below the target size in one attempt you may, for example, use `-brlow 3` to lower the target bitrate by 3% before running FFmpeg.
...
Enables re-encoding the audio with the [selected encoder](Parameters.md#-selectedaudioencoder-alias--ca), even if the input video's audio bitrate is lower than the target audio bitrate.

**Usage:**\
`-ForceAudioEncoding 1` (will always re-encode the audio)

### -PrioritizeAudioBitrate (bool)
**Default:** `0`

Controls whether or not to automatically lower the audio bitrate so the audio takes up 20% or less of the output video file.
...
**Default:** `0` (the first available audio stream).
...
**Default:** `0` (the first available video stream).
...
**Usage:**\
`-videostream 6`\
`-videostream 7`(WHY would you have a file with this many video streams??)
...
### -RetryEncodingPercentageLowAmount (Alias: -retrylow)
**Default:** `-1` (dynamic mode enabled)

When [retrying](Parameters.md#-retryencodingiftargetnotmet-alias--retry-bool) to re-encode the video, lower the target video bitrate by this percentage amount.\
