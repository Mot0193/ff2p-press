...
# Codecs & Advanced settings
By default the script uses the video encoder libx265 (h265) at the "medium" preset, and the audio encoder libopus (opus) at the 120kbps bitrate. You may not agree with these defaults, so feel free to edit the script and change them to fit your needs. The default values are set at the top of the script, where you can see all the parameters and comments explaining them.
...
## libx264 (avc/H264)
H264 is the least efficient when it comes for quality/file size out of these options, but it has the benefit of encoding decently fast (though still slower than hardware encoders). Its the most compatible with devices and services, which means its the best option if you want to guarantee that the video can be played with no issues. 
...
The default is libopus at 128k bitrate (if the input video's bitrate isnt lower). If you're unsatisfied with how it sounds, try AAC. From what I read, opus does great at medium bitrates (64k, 128, 192k), but at very low bitrates you might want to try AAC. At high bitrates the difference between codecs is very minor.